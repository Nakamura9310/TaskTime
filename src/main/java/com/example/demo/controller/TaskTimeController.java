...
import com.example.demo.service.GoogleCalendarAPIService;
...
public class TaskTimeController {
	...
	private OAuth2AuthorizedClient getAuthorizedClient(OAuth2AuthenticationToken authentication) {
		...
		public String displayTop(@AuthenticationPrincipal OAuth2User oauth2User, OAuth2AuthenticationToken authentication, Model model) {
			...
			//ログイン中のuserIDを持つタスクのみ表示
			String userID = oauth2User.getAttribute("email");
			List<Task> task = taskService.selectUndoneTasks(userID);
			...
			model.addAttribute("mon", mon.format(md));
			model.addAttribute("tue", tue.format(md));
			model.addAttribute("wed", wed.format(md));
			model.addAttribute("thu", thu.format(md));
			model.addAttribute("fri", fri.format(md));
			model.addAttribute("sat", sat.format(md));
			model.addAttribute("sun", sun.format(md));
	...
	public String addEvent(OAuth2AuthenticationToken authenticationToken, @PathVariable("taskID") int taskID) throws GeneralSecurityException, IOException {
	...
	@PostMapping("/new")
	public String register(@AuthenticationPrincipal OAuth2User oauth2User, @ModelAttribute Task task) {
		
		//新規タスクinsert
		taskService.insertOneTask(task);

		//emailをuserIDとして登録する処理
		// String userID = oauth2User.getAttribute("email");
		// int taskID = task.getTaskID();//taskIDが0になってしまう
		// taskService.registerUserID(taskID, userID);
